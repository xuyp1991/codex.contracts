...
namespace eosiosystem {
   void system_contract::freeze( const account_name voter, const asset stake ){
      ...
      if( fts == freeze_tbl.end() ) {
         ...
         change -= fts->staked;
         freeze_tbl.modify(fts, _self, [&]( freeze_info& v ) {
            v.staked = stake;
         ...
         auto mv = _voters.find(voter.value);
         if((mv != _voters.end()) && (change != asset{0,CORE_SYMBOL})){
...
// #endif
      ...
      auto create_bp = creation_bp_tbl.find(bpname.value);
         ...
         votes_tbl.modify(vts, _self, [&]( vote_info& v ) {
            ...
            if( change < asset{} ) {
               ...
               if( fts == freeze_tbl.end() ) {
                  ...
                  freeze_tbl.modify(fts, _self, [&]( freeze_info& f ) {
                     f.staked += (-change);
                     f.unstake_height =  curr_block_num;
                  });
               }
   ...
   void system_contract::unfreeze( const account_name voter ) {
      ...
      freeze_table freeze_tbl(_self, _self.value);
      const auto itr = freeze_tbl.get(voter.value, "voter have not freeze token yet");
   ...
   */
   void system_contract::fee( const account_name payer, const account_name bpname, int64_t voteage ) {
      ...
      votes_table votes_tbl(_self, payer.value);
